<template>
  <div>
    ...
    <p class="location"><router-link class="grey" to="/tag">标签管理</router-link>>编辑   <button class="detele-btn" @click="deleteLists" title="删除"></button></p>
    ...
    <div class="wrap-margin wrap-padding">
      <el-form :model="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="封面"> <!--这里不需要 prop="display_type"-->
        ...
        <el-form-item label="新增标签" prop="type">
          ...
          <el-checkbox-group
            ...
            <el-checkbox v-for="item in tags" :label="item.tag_name" :key="item.id" :value="item.id">{{item.tag_name}}</el-checkbox><!--复选框选中后的:label会直接加入v-mode（checkedTags）中,不需要绑定-->
          </el-checkbox-group>
        </el-form-item>
...
export default {
  ...
  watch: {
      ...
      if(this.display_type ==2 || this.display_type ==4){ // 690*388 单张大图
          this.selectedImgs(690,388);
            if(this.cover.length >0){
              ...
              this.imgShow = this.ruleForm.coverage.split(",")// 来源关系：this.ruleForm.article_imgs => this.cover => this.ruleForm.coverage => this.imgShow
       }else{
          this.display_type = 1;
    ...
    checkedTags: function(val) {
      ...
      // });
      // console.log(val);// val为选中复选框值构成数组集合（多出一个内部的__ob__属性,当然forEach无法获取它）
      val.forEach(item => {// // 监听复选框，将复选框所有选中的值拼接到单选框中
        if (this.inputTags.indexOf(item) == -1) {
          this.inputTags = item + "," + this.inputTags;
        }
        this.inputTags = this.inputTags.replace(/,$/,'');// 去掉默认逗号
      });
    }
  },
  methods: {
    ...
    // inputTagsChange(){
    //   this.checkedTags = [];// 需要先清空，否则不能动态监听（无法及时监听到取消选中）
    //   this.tags.forEach(item => {
  ...
  mounted() {
    grabService.sourceList().then(data=>{
      ...
      commonService.tagList().then(data => {
          if (data.code == 0) {
            this.tags = data.data;// 初始化完dom就已经取了tags
            return Promise.resolve();
          }
        });
      ...
      if (data.code == 0) {
        this.ruleForm = data.data;
         // console.log(JSON.stringify(this.ruleForm,["content"]))
        this.imgShow = this.ruleForm.coverage.split(",");
