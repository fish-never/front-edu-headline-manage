      ...
      <el-form :model="ruleForm" label-width="100px" class="demo-ruleForm" :rules="rules">
        ...
        <el-form-item v-if="imgShow.length>=1">
          ...
          <div v-for="(item ,index) in imgShow" :key="index">
            <div v-if="display_type==2 && item.flag==false"  :style="{background:'url('+ item.url +') center center no-repeat #F2F5F7',backgroundSize:'auto 100%'}" class="img-item bg-imgs-big" @click="checkImg(item)" :class="{line: checkCover(item.url)}">
                
            </div>
            <div v-if="display_type==2 && item.flag==true"  :style="{background:'url('+ item.url +') center center no-repeat #F2F5F7',backgroundSize:'100% auto'}" class="img-item bg-imgs-big" @click="checkImg(item)" :class="{line: checkCover(item.url)}">
                
            </div>
            <div v-if="(display_type==3&& item.flag==false) || (display_type==5&& item.flag==false)"  :style="{background:'url('+ item.url +') center center no-repeat #F2F5F7',backgroundSize:'auto 100% '}" class="img-item bg-imgs-small"   @click="checkImg(item)" :class="{line: checkCover(item.url)}">
  
            </div>
             <div v-if="(display_type==3&& item.flag==true) || (display_type==5&& item.flag==true)"  :style="{background:'url('+ item.url +') center center no-repeat #F2F5F7',backgroundSize:'100% auto '}" class="img-item bg-imgs-small"   @click="checkImg(item)" :class="{line: checkCover(item.url)}">
...
export default {
  ...
  data() {
    return {
      ...
      input: "",
      editorText:"",
      img:require("../../assets/imgs/bg.png"),
      inputTags: "",
      checkedTags: [],
      tags: [],
      flag:false,
