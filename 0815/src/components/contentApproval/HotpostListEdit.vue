     ...
     <div class="wrap">
       <el-form :model="fromdata" :rules="rules"  label-width="100px">
            ...
            <el-form-item label="权重" required>
              <el-input v-model="fromdata.weight"></el-input>
            </el-form-item>
  ...
  export default {
      ...
      return {
        ...
        rules: {
          ...
          weight: [
            ...
            { min: 1, message: '权重值不能为空值', trigger: 'blur' }
          ],
          hot: [
    ...
    updating(){
      ...
      postList.update({
          ...
          weight:parseInt(this.fromdata.weight)
          }).then(data => {
              if (data.code == 0) {
                this.toback();
              } else {
    ...
    toback(){
      this.$router.push({path:'/index/contentApproval/HotpostList'});
     }
    }
  }
</script>
